...
class DataLoader():
        ...
        for img_path in batch_images:
            ...
            if not is_testing:
               # img = scipy.misc.imresize(img, self.img_res)
                img = np.array(Image.fromarray(img).self.img_res)
    ...
    def load_batch(self, batch_size=1, is_testing=False):
        ...
        for i in range(self.n_batches-1):
            ...
            for img_A, img_B in zip(batch_A, batch_B):
                ...
                im = Image.open(img_B)
                a = np.asarray(im)
                img_B = Image.fromarray(a)
                plt.imshow(img_B)
                
                #img_A = np.array(Image.fromarray(img_A).resize())
                #img_A = scipy.misc.imresize(img_A, self.img_res)
                
                #img_B = scipy.misc.imresize(img_B, self.img_res)
                # img_B = np.array(Image.fromarray(img_B).resize())
    ...
    def imread(self, path): 
        return np.array(imageio.imread(path), dtype=np.float64) / 255
